...
public class SimpleEngineDao implements EngineDao {
    ...
    public EngineEntity addEngine(String engineName, double engineWeight) throws DaoException {
        try (final Connection connection = connectionPool.getConnection(); final PreparedStatement preparedStatement = connection.prepareStatement(SQL_ADD_ENGINE, Statement.RETURN_GENERATED_KEYS)) {
            ...
            if (countRowsadded > 0) {
                ...
                if (resultSet.next()) {
                    return new EngineEntity.Builder().withId(resultSet.getLong(1)).withEngineName(engineName).withEngineWeight(engineWeight).build();
                }
            }
    ...
    public Optional<EngineEntity> findEngineById(Long id) throws DaoException {
        try (Connection connection = connectionPool.getConnection(); PreparedStatement preparedStatement = connection.prepareStatement(SQL_FIND_ENGINE_BY_ID)) {
            ...
            if (countRowsFind > 0) {
                ...
                if (resultSet.next()) {
                    return Optional.of(new EngineEntity.Builder().withId(id).withEngineName(resultSet.getString(1)).withEngineWeight(resultSet.getDouble(2)).build());
                }
            }
        } catch (SQLException e) {
    ...
    public Optional<EngineEntity> findEngineByName(String engineName) throws DaoException {
        try (Connection connection = connectionPool.getConnection(); PreparedStatement preparedStatement = connection.prepareStatement(SQL_FIND_ENGINE_BY_NAME)) {
            ...
            if (countRowsFind > 0) {
                ResultSet resultSet = preparedStatement.getResultSet();
                if (resultSet.next()) {
                    return Optional.of(new EngineEntity.Builder().withId(resultSet.getLong(1)).withEngineName(engineName).withEngineWeight(resultSet.getDouble(2)).build());
                }
            }
        } catch (SQLException e) {
    ...
    public List<EngineEntity> findAllEngines() throws DaoException {
        ...
        try (Connection connection = connectionPool.getConnection(); Statement statement = connection.createStatement()) {
            ...
            while (resultSet.next()) {
                final EngineEntity engine = new EngineEntity.Builder().withId(resultSet.getLong(1)).withEngineName(resultSet.getString(2)).withEngineWeight(resultSet.getDouble(3)).build();
    ...
    @Override
    public boolean deleteEngineById(Long id) throws DaoException {
    ...
    @Override
    public EngineEntity updateEngine(String engineName, double engineWeight, Long engineId) throws DaoException {
        try (Connection connection = connectionPool.getConnection(); PreparedStatement preparedStatement = connection.prepareStatement(SQL_UPDATE_ENGINE_BY_ID)) {
            ...
            if (countRowsUpdated > 0) {
                return new EngineEntity.Builder().withId(engineId).withEngineName(engineName).withEngineWeight(engineWeight).build();
            }
        } catch (SQLException e) {
